const SECTION_GUIDELINES: Record<string, string> = {
  characters: `Suggested sections (include only those with material):
...
- **Connections**: Relationships to other entities, demonstrated through text`,
  ...
  locations: `Suggested sections (include only those with material):
...
- **Connections**: Links to other locations, characters, or events`,
  ...
  events: `Suggested sections (include only those with material):
...
- **Connections**: Links to characters, locations, or other events`,
  ...
  factions: `Suggested sections (include only those with material):
...
- **Connections**: Links to characters, locations, or other factions`,
  ...
  themes: `Suggested sections (include only those with material):
...
- **Connections**: Links to other themes, characters, or events`,
  ...
  items: `Suggested sections (include only those with material):
...
/**
 * Get section guidelines based on a category path.
 * Extracts the top-level segment (before first '>'), lowercases it,
 * and matches against the guidelines map. Falls back to "other".
 */
export function getSectionGuidelines(category: string): string {
  const topLevel = category.split(">")[0].trim().toLowerCase();
  return SECTION_GUIDELINES[topLevel] || SECTION_GUIDELINES.other;
}
